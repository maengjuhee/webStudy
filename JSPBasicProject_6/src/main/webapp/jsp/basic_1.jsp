...
<%--
      1. JSP 동작 구조
          사용자(클라이언트) : 브라우저 => 서버로 요청 => 파일 요청
                                 ========
                                 URL주소 이용 => 마지막에 파일 첨부
                                 http://localhost:8080/JSPBasicProject_6/jsp/basic_1.jsp
                                                                             ============
                                                                             => 확장자 변경이 가능
      ...
      2. 실행순서
          ...
          5) .class를 읽기 시작
             ...
             = _jspDetroy() => 새로고침, 화면 이동 자동으로 jsp는 메모리에서 해제
             = 서버
                     ...
                     ============ InputStreamReader() => 2byte => Decoding
                                  ===================
                                   request.setCharacterEncoding("UTF-8")
                                                                ========
                                                              = Uncode Transfer Format
                                                                ====== 2byte
      3. HTML과 자바 구분
         <%  %> : 일반 자바 소스 => 지역변수 선언 , 연산자 , 제어문 , 메소드 호출
                  문법사항이 자바와 동일
                  문장 종료 ;
                  주석 : // 한 줄 주석 
                       /* */ 여러줄 주석
      ...
      4. 지시자 5장
          ...
          = include : 정적 => 소스를 통합한 후에 컴파일
               => 메뉴 / Footer
               <%@ include file=""%>
               JSP안에 특정 부분에 다른 JSP를 포함할 때 사용 => 조립식
          = taglib => 20장에 
          *** <%@ 지시자 속성="값" %> => ""를 반드시 사용한다
      5. 액션 태그 6장
          151page
