       ...
       = ASP, PHP
         ...
         2. 사용자 요청 정보
           ...
           ***String getParameter() : 단일값을 받을 때 사용
              String[] getPatameterValues() : checkbox, select => multiple
...
<%--
     ...
     묶는 방법 => Map 방식이다
               ====== 키, 값 => getParameter("키")
               list.jsp?num=1&num=2&num=3&4&5   
     setAttribute(키,값); 키는 중복이 불가능하다..
     
     => getRequestURI()
        => /프로젝트 폴더명/폴더명/파일명?id=admin%pwd=1234  
                                ================= request첨부
          ====================== URI                                     
 --%>
</head>
      ...
      <form method="post" action="request_ok.jsp">
       <table class="table">
          ...
          <tr>
           <th width=20%>전화</th>
           <td width=80%>
             <select name="phone" class="input-sm">
               <option>010</option>
               <option>011</option>
             </select>
          ...
          <tr>
            <td cospan="2" class="text-center">
              <input type="submit" value="전송" class="btn-sm bte-primary">
            </td>
          </tr>
         </form>
       </table>
