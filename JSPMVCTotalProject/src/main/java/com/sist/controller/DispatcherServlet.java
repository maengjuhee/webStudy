 ...
 *       = 장점
 *          1) 서블릿 => 자바 코드만으로 HTML 코드 작성 => 오류 발생할 확륭이 낮다
 *          2) JSP => 자바 코드만으로 HTML 코드 작성 => 오류 발생할 확륭이 낮다
 *          3) 단순하다 => 자바/HTML을 구분
 *                      <% %>, <%= %> => 제어문 : for / if 
 *       = 단점 
...
public class DispatcherServlet extends HttpServlet {
    ...
    private String[] strCls= {
    		...
    		"com.sist.model.MovirDetailModel"
    };
	...
	public void init(ServletConfig config) throws ServletException {
		...
		try
		{
			for(int i=0;i<strCls.length;i++)
			{
				...
				// 클래스 메모리 할당
				// new => 결합성이 높다 (new 사용을 자제) => 스프링에서는 new를 사용하지 않는다
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("model2="+model1);
		System.out.println("model3="+model1);
		System.out.println("model4="+model1);
		System.out.println("model5="+model1);*/
		// 사용자 요청 사항
