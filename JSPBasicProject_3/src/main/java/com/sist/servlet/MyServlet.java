 ...
 *                    Java(Setvlet), JSP => WAS (Web Application Server)
 *                                         ============================= Tomcat
 *                                         a.jsp 
 *                                         =====
 *                                         1. 클래스로 변경
 ...
 *                                4. 서블릿이 종료 (새로고침, 화면 이동)> destroy() 호출
 *                     =======
 *                     out.write("") => 메모리에 출력
 *                                      ======== 
 ...
 *                                        | 이미지 / 애니메이션 => 2000년 웹 => 전자 상거래
 *                         => 웹 서비스 기능을 해주는 자바 클래스
 *                            => 자바 안에 HTML코드를 첨부 (JSP => HTML 안에 Java 첨부)
 *                               out.write("<html>") => <%
 *                                                      %>
 *                                                      <html>
 *                            => 단점
 ...
 *                                      ==================== 전체 소스를 볼 수 없다, 다른 사럼아 변경이 불가능
 *                                주로 사용처 : HTML / 자바가 분리 된 경우 => 연결한 역할
 *                                         =============================== Controller
 *                                                                          | 스프링은 이미 제작 (서블릿)
 *                            => 단점을 보완
 ...
 *                   자바 => 썬 => package => javax
 *                        ====
 *                         | 오라클 => package 명칭이 => jakarata 변경 => tomcat 10 이상
 *                         STS 3.9 => jdk 11 -> 실무 1.8(호환성이 좋다)
 *                         STS 4.X => jdk 17
 *                         => 전자 정부 프레임워크 / ANY 프레임워크
 *                            => 공기업         => 대기업 ==> STS 3.XXX
...
public class MyServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		String path=config.getInitParameter("file name");
		System.out.println();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 디폴트 => 메인화면 =>화면 UI
		// 1. 변환 => 브라우저에 알려주는 내용 (HTML,XML,JSON) => JSON => JSP (page 지시자)
